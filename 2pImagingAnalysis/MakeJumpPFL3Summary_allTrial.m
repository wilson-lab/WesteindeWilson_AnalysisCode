function MakeJumpPFL3Summary_allTrial(rootDir, startAtMov)       
% Summarize PFL3 responses following jumps 
...
% jump and collect window starting at that point. 
    ...
    % Collect experimental folders
     ...
     headers = {'folder','trial','jumpSize','jumpIdx','velFor','velSide','velYaw','cueAngle','corrected','timeMov','Z','region'};
       ...
       % Load in data
        try
            ...
            for nTrial = 1:numTrials
                ...
                if size(ZData,1) == 2 && ~ismembertol(rho_fullTrial,1,10^-10) % if rho = 1 something is wrong & should only have 2 rois for LAL
                    window = 15; 
                    ...
                    for jump = 1:size(jump_array,1)
                        if startAtMov % collect window after jump starting when fly started moving
                            ...
                            % Calculate R-L activity difference
                            jump_Z = all_roi(2,:) - all_roi(1,:);
                        end
    ...
    for jump = 1:size(Sum180,1)  
        if ~isempty(Sum180.jumpIdx{jump})
            ZSum(jump,:) = smoothdata(abs(Sum180.Z{jump}),'loess',smoothWin);
    ...
    for jump = 1:size(Sum180_not,1) 
        if ~isempty(Sum180_not.jumpIdx{jump})
            ZSum(jump,:) = smoothdata(abs(Sum180_not.Z{jump}),'loess',smoothWin);
    ...
    for jump = 1:size(Sum90,1) 
        if ~isempty(Sum90.jumpIdx{jump})
            ZSum(jump,:) = smoothdata(abs(Sum90.Z{jump}),'loess',smoothWin);
    ...
    for jump = 1:size(Summ90,1)
        if ~isempty(Summ90.jumpIdx{jump})
            ZSum(jump,:) = smoothdata(abs(Summ90.Z{jump}),'loess',smoothWin);
    ...
    for jump = 1:size(Sum90_not,1) 
        if ~isempty(Sum90_not.jumpIdx{jump})
            ZSum(jump,:) = smoothdata(abs(Sum90_not.Z{jump}),'loess',smoothWin);
    ...
    for jump = 1:size(Summ90_not,1)
        if ~isempty(Summ90_not.jumpIdx{jump})
            ZSum(jump,:) = smoothdata(abs(Summ90_not.Z{jump}),'loess',smoothWin);
    ...
    for jump = 1:size(Summ90,1)  
        if ~isempty(Summ90.jumpIdx{jump})
            ...
            vfSum(jump,:) = smoothdata(Summ90.velFor{jump}','loess',smoothWin_v); 
            vySum(jump,:) = smoothdata((Summ90.velYaw{jump}'),'loess',smoothWin_v);
        end
    end
    SEM_vf = std(vfSum,[],1,'omitnan') / sqrt(size(vfSum,1));
    SEM_vy = std(vySum,[],1,'omitnan') / sqrt(size(vfSum,1));
    ...
    SEMhigh = [mean(vfSum(:,keepIndex),'omitnan') + SEM_vf(keepIndex)]; 
    SEMlow = [mean(vfSum(:,keepIndex),'omitnan') - SEM_vf(keepIndex)];
    plot(cx2,jumpTime,mean(vfSum,'omitnan'),'k','LineWidth',1.5)
    ...
    SEMhigh = [mean(vySum(:,keepIndex),'omitnan') + SEM_vy(keepIndex)]; 
    SEMlow = [mean(vySum(:,keepIndex),'omitnan') - SEM_vy(keepIndex)];
    plot(cx3,jumpTime,mean(vySum,'omitnan'),'k','LineWidth',1.5)
    ...
    for jump = 1:size(Sum90,1)  
        if ~isempty(Sum90.jumpIdx{jump})
            ZSum(jump,:) = smoothdata(Sum90.Z{jump},'loess',smoothWin_v);
            vfSum(jump,:) = smoothdata(Sum90.velFor{jump}','loess',smoothWin_v); 
            vySum(jump,:) = smoothdata((Sum90.velYaw{jump}'),'loess',smoothWin_v);
        end
    end
    SEM_vf = std(vfSum,[],1,'omitnan') / sqrt(size(vfSum,1));
    SEM_vy = std(vySum,[],1,'omitnan') / sqrt(size(vfSum,1));
    ...
    SEMhigh = [mean(ZSum(:,keepIndex),'omitnan')+ SEM_Z(keepIndex)]; 
    SEMlow = [mean(ZSum(:,keepIndex),'omitnan') - SEM_Z(keepIndex)];
    plot(cx1,jumpTime,mean(ZSum,'omitnan'),'b','LineWidth',1.5)
    patch(cx1,[jumpTime(keepIndex) fliplr(jumpTime(keepIndex))],[SEMhigh fliplr(SEMlow)],[0,0,0.75],'FaceAlpha',0.25,'EdgeColor','none')
    ylabel(cx1,'Z')
    xlabel(cx1,'Time (s)')
    xlim(cx1,[min(jumpTime),max(jumpTime)])
    ...
    SEMhigh = [mean(vfSum(:,keepIndex),'omitnan') + SEM_vf(keepIndex)]; 
    SEMlow = [mean(vfSum(:,keepIndex),'omitnan') - SEM_vf(keepIndex)];
    plot(cx2,jumpTime,mean(vfSum,'omitnan'),'b','LineWidth',1.5)
    patch(cx2,[jumpTime(keepIndex) fliplr(jumpTime(keepIndex))],[SEMhigh fliplr(SEMlow)],[0,0,0.75],'FaceAlpha',0.25,'EdgeColor','none')
    xlim(cx2,[min(jumpTime),max(jumpTime)])
    ylabel(cx2,'forward velocity (mm/s)')
    xlabel(cx2,'Time (s)')
    ...
    SEMhigh = [mean(vySum(:,keepIndex),'omitnan') + SEM_vy(keepIndex)]; 
    SEMlow = [mean(vySum(:,keepIndex),'omitnan') - SEM_vy(keepIndex)];
    plot(cx3,jumpTime,mean(vySum,'omitnan'),'b','LineWidth',1.5)
    patch(cx3,[jumpTime(keepIndex) fliplr(jumpTime(keepIndex))],[SEMhigh fliplr(SEMlow)],[0,0,0.75],'FaceAlpha',0.25,'EdgeColor','none')
    xlim(cx3,[min(jumpTime),max(jumpTime)])
    ylabel(cx3,'rotational velocity (deg/s)')
    xlabel(cx3,'Time (s)')
